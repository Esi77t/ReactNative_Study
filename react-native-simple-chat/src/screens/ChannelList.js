...
const Item = React.memo(({ item: { id, title, description, createAt }, onPress }) => {
        ...
        const getDateOrTime = ts => {
            // startOf(unit) : 현재 moment 객체를 지정한 단위의 시작 지점으로 변경한다
...
const ChannelList = ({ navigation }) => {
    ...
    // 채널 목록 화면이 렌더링 됐을 때 콜백함수를 실행하겠다
    useEffect(() => {
        // 1. firestore의 'channels' 컬렉션에 대한 쿼리 생성
        // 생성일을 내림차순을 정렬해서 데이터를 가져온다
        const collectionQuery = query(
            collection(db, 'channels'), orderBy('createAt', 'desc')
        )

        // 2. onSnapshot : 실시간 리스너 등록
        // 수신 대기 상태로 있다가 데이터베이스에 문서가 추가되거나 수정될 때마다 지정된 함수가 호출된다
        ...
        // 5. useEffect의 클린업 함수
        // 컴포넌트가 언마운트 되었거나, 리렌더링으로 이 이펙트가 재실행될 때 기존 구독을 해제한다
        // 수신 대기상태를 해제하지 않으면, 다시 채널 목록 화면이 마운트될 때 수신 대기 이벤트가 추가되면서
        // 데이터를 중복으로 받아오는 문제가 발생할 수 있다
        return () => unsubscribe();

        // 6. 의존성 배열이 비어 있으므로 ([]), 이 이펙트는 컴포넌트 마운트 시 한 번만 실행된다. 
    }, [])
